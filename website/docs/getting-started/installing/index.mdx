---
title: Installing
description: Install Scratch Addons from extension stores or from source.
sidebar_position: 2
---
...
This is the recommended way to install Scratch Addons for development purposes.
...
This will update Scratch Addons and get it ready for code editing.
Note that you will need to see the finish updating section [here](#install-on-google-chrome) if you are using Google Chrome.

### Downloading the zipball

If you don't have Git installed, you can try this method instead.
Note that you will need to manually repeat this process every time you want to update Scratch Addons.

1. Go to the [repository](https://github.com/ScratchAddons/ScratchAddons) and find the download code button.

   ![Download code button screenshot](./download-code-button.png)

2. Click it and select "Download ZIP".

   ![Download ZIP button screenshot](./download-zipball-button.png)
...
2. Click the toggle next to `Developer mode` to turn on the Developer Mode. This allows you to install extensions from a folder or file.
   ...
   ![Extension Management top bar screenshot](./developer-mode-toggle.png)

3. You should see the `Load unpacked` button appear. Clicking it will allow you to select a folder to upload.

   ![Load unpacked button screenshot](./load-unpacked-button.png)
...
![Update button screenshot](./update-button.png)
...
2. Click `This Firefox` on the left-hand menu.
   ...
   ![Left-hand menu screenshot](./left-hand-menu.png)

3. Click `Load Temporary Add-on...`.

   ![Load Temporary Add-on button screenshot](./load-addon.png)

4. Select the manifest.json file inside the extracted folder.
5. The extension should now be loaded.

Note: Firefox temporary add-ons are actually temporary.
Restarting Firefox will remove them, so if you want to use the development version of Scratch Addons all the time,
it is recommended that you use a Chromium-based browser like Google Chrome.
