...
Using `addon.tab.waitForElement`, we don't need to have code that waits until comments have loaded.
This way, we also store the IDs of newly posted comments and new loaded comments when the user clicks "load more comments".

```js
const commentIds = [];
while (true) {
  const comment = await addon.tab.waitForElement("div.comment", { markAsSeen: true });
...
We create a button to hide the image when clicked, and the image successfully gets hidden, even if the addon is enabled.
We also set the `display` CSS property of the image to `flex` when visible, even tho that is not the default value for images.

```js title="userscript.js"
...
```css title="userstyle.css"
.sa-example-img {
  display: flex;
}

.sa-example-img-hide {
  /* We want to hide the image if the button was clicked, even if the addon is enabled */
...
addon
  .tab
  .addEventListener("urlChange", event => console.log(`URL changed! It was previously ${event.detail.oldUrl}, it's now ${event.detail.newUrl}`));
```
...
Options `condition`, `reduxCondition` and `reduxEvents` should be used as optimizations,
in order to avoid multiple calls to `document.querySelector` when it's guaranteed the element will not exist yet.
...
If the intended `display` CSS property value for the provided element when visible is not the default value for the type of provided element
(for example, `block` for `div`s and `inline` for `span`s), you should provide that value inside the options parameter.
If you want to manually hide the element in situations where the addon is enabled, you should use a dedicated class name for that,
instead of manually setting `el.style.display = "none";`. Use a class name selector in a userstyle to set `display: none !important;` on the element.
...
### [`addon.tab.appendToSharedSpace`](./addon.tab.appendToSharedSpace/index)

See [addon.tab.appendToSharedSpace](./addon.tab.appendToSharedSpace/index)
...
<table>
  ...
  <tr>
    ...
    <td>No</td>
    <td>
      Show the context menu item in various locations.
      <table>
        <tr>
          <th>Property</th>
          <th>Type</th>
...
<table>
  ...
  <tr>
    ...
    <td>Yes</td>
    <td>
      The function that is executed when the item is clicked.
      <table>
        <tr>
          <th>Parameter</th>
          <th>Type</th>
    ...
    <td>
      <table>
        ...
        <tr>
          ...
          <td>true</td>
          <td>
            A function that determines if the item should be shown.
            <table>
              <tr>
                <th>Parameter</th>
                <th>Type</th>
        ...
        <tr>
          ...
          <td>
            ...
            <table>
              ...
              <tr>
                <td>args</td>
                <td>An object containing the values that are entered into the block inputs. The keys correspond to the `args` property specified earlier.</td>
              </tr>
              <tr>
                <td>thread</td>
